...
const DateSelection = ({ date, selectedYear, selectedMonth, onSelectDate }) => {
    ...
    const renderDays = () => {
        ...
        for (let i = 0; i < firstDayOfMonth; i++) {
            days.push(<div key={`empty-${i}`} className="empty-day">{getDayOfPrevMonth + i}</div>);
        }
        ...
        for (let i = 0; i < visibleNumberOfDaysFromNextMonth; i++) {
            days.push(<div key={`empty-next-${i}`} className="empty-day">{i + 1}</div>);
        }
        return days;
    };
        ...
        <>
            <div className="day-labels">
                {dayNames.map((day, index) => {
                    return (<div className="label" key={index}>{day}</div>);
                })}
    ...
    return (
        <div className="month-list">
            {monthNames.map((month, index) =>
                <div className="month label" onClick={() => onSelectMonth(index)}>{month}</div>)}
        </div>
    );
};
const YearSelection = ({ selectedYear, onSelectYear }) => {
    ...
    const getYearDecade = (firstYearDecade) => {
        let yearList = [];
        console.log('firstYearDecade', firstYearDecade);
        for (let i = 0; i < 10; i++) {
    ...
    return (
        <div className="year-decade-list">
            {yearList?.map((year) =>
                <div className="year label" key={year} onClick={() => onSelectYear(year)}>{year}</div>)}
        </div>
    );
};
            ...
            <div className="header">
                ...
                <div className="month-year">
                    { selectionTab === 'day' &&
                      ...
                      <a onClick={() => changeSelectedTab('month')}>{`${monthNames[selectedMonth]} ${selectedYear}`}</a> }
                    { selectionTab === 'month' &&
                      <a onClick={() => changeSelectedTab('year')}>{selectedYear}</a> }
                    { selectionTab === 'year' &&
